...
def _input_actions():
    ...
    try:
        actions = []
        num_actions = int(input('Enter the Number of Metrics: '))
        assert 1 <= num_actions
        set_actions = ['STEPS','LATLONG','TOTALTIME','TOTALDISTANCE','ELEVATION']
        while len(actions)!= num_actions:
            try:
                action = input('Enter Action from {}: '.format(set_actions))
                action = action.upper()
                if action.strip() in ['STEPS','LATLONG','TOTALTIME','TOTALDISTANCE','ELEVATION']:
                    actions.append(action.strip())
                    set_actions.remove(action.strip())
                else:
                    _invalid_move_message()
            except IndexError:
                _invalid_move_message()
